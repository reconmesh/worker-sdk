...
Default if unset: 1 (one in-flight per host). Conservative on purpose
· raise if your worker is read-only (DNS lookups, passive sources)
or your target tolerates concurrent connections (CDNs).
